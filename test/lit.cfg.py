...
config.test_exec_root = os.path.join(config.pcl_obj_root, 'test')
...
# test_exec_root: The root path where tests should be run.
config.test_exec_root = os.path.join(config.pcl_obj_root, 'test')
config.pcl_tools_dir = os.path.join(config.pcl_obj_root, 'bin')

# Tweak the PATH to include the tools dir.
llvm_config.with_environment('PATH', config.pcl_tools_dir, append_path=True)
llvm_config.with_environment('PATH', config.llvm_tools_dir, append_path=True)

tool_dirs = [config.pcl_tools_dir, config.llvm_tools_dir]
tools = ["pcl-opt"]

llvm_config.add_tool_substitutions(tools, tool_dirs)

llvm_config.with_environment('PYTHONPATH', [
    os.path.join(config.mlir_obj_dir, 'python_packages', 'pcl'),
], append_path=True)
